#! /bin/bash
### r10e,r11c,r12b,r13b,r14b,r15c,r16b,r17c,r18b,r19
NDK=r18b
#### true|false
LOG=true
####
...
rej=($sources/$cam/*.rej)
[ -e "${rej[0]}" ] && dialog --title "ERROR!" --msgbox '                PATCH ERROR! '$cam'' 5 60 && rm -rf $sources/$cam && exit;
fi
echo "DIR := $sources" >> $sources/config.mk
echo "CAM := $cam" >> $sources/config.mk
echo "SRC_FILES := \\" >> $sources/config.mk
find $cam -name '*.c' ! -name 'tests*' ! -name 'tommy*' ! -name 'stream*' ! -name 'config.c' ! -name 'pages*' ! -name 'list_smargo*' -printf '\t%p \\\n' | sort >> $sources/config.mk
echo "	$cam/webif/pages.c \\" >> $sources/config.mk
echo "	$cam/config.c" >> $sources/config.mk
echo "" >> $sources/config.mk
...
NDK_REV=${NDK:1:2};
if [ "$NDK_REV" -le "13" ] ; then
export PATH=$ANDROID_NDK/toolchains/$TOOLCHAINS-4.9/prebuilt/linux-$M_TYPE/bin:$PATH
else
export PATH=$ANDROID_NDK/toolchains/llvm/prebuilt/linux-$M_TYPE/bin:$PATH
fi
